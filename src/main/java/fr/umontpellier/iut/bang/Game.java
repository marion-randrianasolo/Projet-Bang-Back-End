...
public class Game {
    ...
    public int getPlayerDistance(Player player1, Player player2) {
        ...
        int distance;
        int distance2;
        while(players.get(numplayeur1)!=player1){
            numplayeur1++;
        }
        while(players.get(numplayeur2)!=player2){
            numplayeur2++;
        }

        if( numplayeur2>numplayeur1){
            distance= numplayeur2-numplayeur1;
            distance2=numplayeur1+(players.size()-numplayeur2);
        }else{
            distance = numplayeur1-numplayeur2;
            distance2=numplayeur2+(players.size()-numplayeur1);
        }
    ...
    public void removePlayer(Player p) {
        if(playeurVultureSam()!=null){
            Player player = playeurVultureSam();
            for( Card card : p.getHand()){
                player.addToHand(card);
            }
            for(BlueCard blueCard: p.getInPlay()){
                player.addToHand(blueCard);
            }
        }
        if (p == currentPlayer) {
    ...
    private void updateGameFinished() {
        if (sheriffPlayer.isDead()){
            ...
            else if (players.contains(renegadePlayer)){
            ...
            for (Player e : outlawPlayers){
            if(e.isDead()&& renegadePlayer.isDead()){
    ...
    public void remettreDansPioche(Card c){
        drawPile.push(c);

    }
    public Player playeurVultureSam(){
        for (Player player: players){

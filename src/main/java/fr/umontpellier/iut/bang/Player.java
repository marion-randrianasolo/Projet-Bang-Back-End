...
public class Player {
    ...
    public List<Card> getHand() {
        for(Player player: game.getPlayers()){
            if(hand.size() ==0 && player.bangCharacter.getName().equals("Suzy Lafayette")) {
                drawToHand();
            }
        }
        return hand;
    ...
    public List<Player> getOtherPlayers() {
        List<Player> listPlayer = new ArrayList<>(game.getPlayers());
        int i = getGame().getPlayers().indexOf(this);

        List<Player> debut =new ArrayList<>();
        for(int a =0; a<i;a++){
            debut.add(listPlayer.get(a));
        }
       List<Player> fin = new ArrayList<>();
        for(int b =i+1; b<game.getPlayers().size();b++){
            fin.add(listPlayer.get(b));
        }
        fin.addAll(debut);
        return fin;
    }
    ...
    public List<Player> getPlayersInRange(int range) {
        List<Player> listPlayerInRange = new ArrayList<>();
        for(Player player : getOtherPlayers()){
            if(this.distanceTo(player)<=range){
                listPlayerInRange.add(0,player);
            }
        }

        return listPlayerInRange;
    }

    public int getPlayersInRange2(int range) {
        int i=0;
        for(Player player : getOtherPlayers()){
            if(this.distanceTo(player)<=range){
                i++;
    ...
    public void decrementHealth(int n, Player attacker) {
        ...
        if(healthPoints<=0){
            for (BlueCard c : inPlay) {
                if (c.getName().equals("Beer")) {
                    c.canPlayFromHand(this);
                }
            }
        }

        if(bangCharacter.getName().equals("El Gringo") && attacker!=null && !this.isDead()){
            int i =0;
            while(attacker.hand.size()!=0 && i<n){
        ...
        if(bangCharacter.getName().equals("Bart Cassidy") && !this.isDead()){
            int i =0;
            while(i<n){
                drawToHand();
                i++;
    ...
    public int distanceTo(Player player) {
        ...
        for (BlueCard c : player.inPlay) {
            if (c.getName().equals("Mustang")) {
                distance++;
            }
        }
        if(player.bangCharacter.getName().equals("Paul Regret")){distance++;}

        if(bangCharacter.getName().equals("Rose Doolan") && distance>1){
            distance--;
        }
        for(BlueCard d : inPlay){
            if(d.getName().equals("Scope") && distance>1){
                distance--;
            }
        }
        return distance;
    ...
    public void removeFromInPlay(String nameCard){
        BlueCard card = getCardInPlay(nameCard);
        removeFromInPlay(card);
    }
    ...
    public void playTurn() {
        ...
        if(inPlay.size()>0  &&  getCardInPlay("Dynamite")!=null ){
            BlueCard dynamite = getCardInPlay("Dynamite");
            Card card1 = randomDraw();
            if(card1.getSuit()==CardSuit.SPADE && card1.getValue()<=9){
        ...
        if(getCardInPlay("Jail")==null || (getCardInPlay("Jail")!=null && randomDraw().getSuit()==CardSuit.HEART)){

            if (getCardInPlay("Jail")!=null ){
                Card test = getCardInPlay("Jail");
                discard(test);
            ...
            while (true) {
                ...
                if (compteur==0) {
                ...
                } else if (getWeapon() != null && getWeapon().getName().equals("Volcanic") || bangCharacter.getName().equals("Willy the Kid")) {
                    List<Card> possibleCards = new ArrayList<>();
                    for (Card c : hand) {
                        if (c.canPlayFromHand(this)) {
                            possibleCards.add(c);
                    ...
                    List<Card> possibleCards = new ArrayList<>();
                    for (Card c : hand) {
                        if (c.canPlayFromHand(this) && !c.getName().equals("Bang!")) {
                            possibleCards.add(c);
                        }
                    }
                    Card card = chooseCard("Choisissez une carte à jouer", possibleCards, false, true);
    ...
    public Card getCardInHand(String card) {
        for (Card c : getHand()) {
